...
export const Footer = ({ onOpenModal }: FooterProps) => {
  return (
    <footer className="border-t border-white/5 bg-black/60">
      <div className="section-container flex flex-col items-center gap-6 text-center text-sm text-parchment/70 md:flex-row md:justify-between md:text-left">
        <div>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {(["Contact", "Privacy Policy", "Terms"] as const).map((label) => (
            <button key={label} className="text-sm text-parchment/80 transition hover:text-gold" onClick={() => onOpenModal(label)}>
              {label}
            </button>
          ))}
        </div>
